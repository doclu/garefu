  ...
  <div>
    <div class="pop-plogin">
      ...
      <div class="phone-wrapper border-1px phone-wrapper-hook">
        <input
          type="tel"
          class="input-val phone-hook needsfocus"
          placeholder="请输入手机号码"
          maxlength="11"
          v-model="telephone"
        >
      ...
      <div
        class="login-btn btn-hook"
        v-show="!eventActive"
        :class="isTelephoneVal? 'bg-active': 'bg-base'"
      >下一步</div>
      <div
        class="login-btn btn-hook"
        v-show="eventActive"
        :class="isTelephoneVal? 'bg-active': 'bg-base'"
        @click="toMessageConfirm"
      >下一步</div>
    </div>
...
export default {
  ...
  computed: {
    ...mapState(["isTelephoneVal"])
  },
  methods: {
    ...mapActions(["close_mask", "toMessageConfirm"])
  }
};
</script>
<style>
